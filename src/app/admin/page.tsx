...
export default async function AdminHome() {
  ...
  const [leadsResult, projectsResult, financeResult, subscriberResult] = await Promise.all([
    ...
    supabase.from('finance_transactions').select('id,created_at,occurred_on,type,status,amount,category,description,reference,client_id,project_id,created_by').order('occurred_on', { ascending: false }),
