...
export default async function AdminHome() {
  ...
  const { error: attentionRefreshError } = await supabase.rpc('crm_refresh_attention_items');
  if (attentionRefreshError && !['42P01', '42883'].includes(attentionRefreshError.code)) {
    console.error('Attention inbox refresh failed.', attentionRefreshError);
  }

  const [leadsResult, projectsResult, financeResult, subscriberResult, attentionResult] = await Promise.all([
    ...
    supabase.from('attention_items').select('id,kind,priority,status,title,detail,owner_email,due_at,snoozed_until,source_type,source_id,source_href,action_label,idempotency_key,completed_at,created_at,updated_at').in('status', ['open', 'snoozed']),
  ]);
  ...
  if (attentionResult.error && attentionResult.error.code !== '42P01') console.error('Attention overview query failed.', attentionResult.error);
  ...
  return (
    <AdminShell member={member}>
      <OverviewDashboard
        leads={leads}
        projects={(projectsResult.data ?? []) as CrmProject[]}
        finances={(financeResult.data ?? []) as FinanceTransaction[]}
        subscriberCount={subscriberResult.count ?? 0}
        attentionItems={(attentionResult.data ?? []) as AttentionItem[]}
      />
      <RealtimeRefresh tables={['leads', 'crm_projects', 'finance_transactions', 'newsletter_subscribers', 'attention_items']} />
    </AdminShell>
  );
}
