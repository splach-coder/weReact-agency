...
export default async function FinancePage() {
  ...
  const [transactionResult, projectResult] = await Promise.all([
    supabase.from('finance_transactions').select('id,created_at,occurred_on,type,status,amount,category,description,reference,client_id,project_id,created_by').order('occurred_on', { ascending: false }).order('created_at', { ascending: false }),
    supabase.from('crm_projects').select('id,project_name').order('project_name'),
  ]);
