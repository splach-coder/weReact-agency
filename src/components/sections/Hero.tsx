...
export default function Hero() {
  return (
    <section className="relative min-h-screen overflow-hidden px-4 pb-14 pt-28 text-[var(--color-primary)] md:px-6 md:pt-32">
      ...
      <div className="relative z-10 mx-auto grid min-h-[calc(100vh-8rem)] w-full max-w-[1400px] gap-6 lg:grid-cols-[1.02fr_0.98fr] lg:items-stretch">
        <motion.div
          initial={{ opacity: 0, y: 24 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }}
          className="flex min-w-0 w-full max-w-full flex-col justify-between gap-8 rounded-[8px] border border-[var(--color-primary)]/14 bg-[var(--color-background-main)]/70 p-5 backdrop-blur-sm md:p-8 lg:p-10"
        >
          <div>
            <OpsBadge tone="success">Marrakech website command center</OpsBadge>
            <h1 className="mt-7 max-w-5xl break-words text-[2.65rem] font-black uppercase leading-[0.88] tracking-normal text-[var(--color-primary)] sm:text-5xl md:text-7xl lg:text-[6.7rem]">
              Websites built like growth systems.
            </h1>
            <p className="mt-7 max-w-2xl break-words text-base leading-relaxed text-[var(--color-text-secondary)] md:text-xl">
          ...
          <div className="grid gap-4">
            <div className="flex flex-col gap-3 sm:flex-row">
              <Link
                ...
                className="group inline-flex min-h-14 w-full min-w-0 items-center justify-center gap-3 rounded-[6px] bg-[var(--color-primary)] px-5 text-sm font-black uppercase tracking-[0.14em] text-white transition hover:-translate-y-0.5 hover:bg-[var(--color-primary-dark)] sm:w-auto sm:px-7"
              >
                Start a project
              ...
              <Link
                href="#work"
                className="inline-flex min-h-14 w-full min-w-0 items-center justify-center gap-3 rounded-[6px] border border-[var(--color-primary)]/18 bg-white/70 px-5 text-sm font-black uppercase tracking-[0.14em] text-[var(--color-primary)] transition hover:-translate-y-0.5 hover:border-[var(--color-primary)] sm:w-auto sm:px-7"
              >
                View systems
                <ArrowRight size={17} />
              </Link>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              {metrics.map((metric) => (
                <div key={metric.label} className="min-w-0 rounded-[6px] border border-[var(--color-primary)]/14 bg-white/60 p-3">
        ...
        <motion.div
          ...
          className="grid min-w-0 w-full max-w-full gap-4"
        >
